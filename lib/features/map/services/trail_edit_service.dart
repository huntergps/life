...
/// Service for offline field editing of trails, including creation and
...
class TrailEditService {
  ...
  Future<Trail?> updateTrailCoordinates({
    ...
    try {
      ...
      final simplified = GeoUtils.simplifyTrack(newCoordinates,
          toleranceMeters: rdpTolerance);
      AppLogger.info(
          'GPS points: ${newCoordinates.length} → simplified: ${simplified.length}');
      ...
      AppLogger.info(
          'Trail $trailId coordinates updated (${simplified.length} pts, $distance km)');
  ...
  Future<Trail?> createNewTrail({
    ...
    final simplified =
        GeoUtils.simplifyTrack(coordinates, toleranceMeters: rdpTolerance);
    AppLogger.info(
        'GPS points: ${coordinates.length} → simplified: ${simplified.length}');
    ...
    try {
      ...
      for (final item in list) {
        ...
        try {
          ...
          AppLogger.warning(
              'Could not sync pending trail "${data['name_en']}": $e — will retry');
      ...
      if (synced > 0) {
        AppLogger.info(
            'Synced $synced pending trail(s), ${remaining.length} still queued');
      }
    } catch (e, st) {
