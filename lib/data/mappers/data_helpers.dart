...
// ---------------------------------------------------------------------------
// Non-mobile: Supabase direct fetch (desktop / web -- Drift not initialized)
// ---------------------------------------------------------------------------

/// Queries Supabase directly when Drift is not available (desktop/web).
...
/// Fetches a list from the repository with local-first strategy.
///
...
Future<List<T>> fetchDeduped<T extends OfflineFirstWithSupabaseModel>({
  ...
  // Desktop / web: Drift not initialized -- query Supabase directly.
  if (!WildlifeRepository.initialized) {
  ...
  final repo = WildlifeRepository.instance;
  ...
  if (awaitRemote) {
    ...
    // awaitRemoteWhenNoneExist: returns local data immediately if available,
    // otherwise awaits Supabase. This ensures web (empty IndexedDB on first
    // load) gets real data instead of an empty list.
    raw = await repo.get<T>(
      policy: OfflineFirstGetPolicy.awaitRemoteWhenNoneExist,
      query: query,
    );

    // Fire background sync to keep local cache fresh for subsequent reads.
...
/// Builds a lookup map from repository results, deduplicating by [idSelector].
Future<Map<int, T>> fetchLookup<T extends OfflineFirstWithSupabaseModel>({
  ...
  final raw = await WildlifeRepository.instance.get<T>(policy: policy, query: query);
  final map = <int, T>{};
  for (final item in raw) {
    map[idSelector(item)] = item;
  }
